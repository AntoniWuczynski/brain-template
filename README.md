...
- **The `brain-project-note` skill** captures the project or session you are
  working in — from any repository, or a chat-based project with none — into
  `knowledge/projects/<slug>/` (or `knowledge/chats/<slug>/`), keyed by git
  remote so one project maps to one folder. It writes only through MCP and
